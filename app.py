...
def get_db_connection():
    return mysql.connector.connect(
        ...
        password="password", # Contraseña para ingresar a MySQL
        database="cafe_db", # Nombre de la base de datos
...
def token_required(f):
    ...
    def decorator(*args, **kwargs):
        ...
        try:
            ...
            print("El token ha expirado") # Imprime en la consola y terminal cuando el token ha expirado
            return jsonify({'message': 'Token expirado'}), 401

        except jwt.InvalidTokenError:
            print("Token inválido") # Imprime en la consola cuando el token es inválido
            return jsonify({'message': 'Token inválido'}), 401
        except Exception as e:
            print("Error al procesar el token") # Imprime en la consola cuando hay error al procesar el token
